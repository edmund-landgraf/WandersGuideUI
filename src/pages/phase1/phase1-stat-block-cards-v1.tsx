...
export function Phase1StatBlockCardsV1({ entity, data }: { entity: LivingEntity; data: StatBlockContent }) {
    ...
    <div className='stat-block-experimental min-w-[1100px] space-y-2 pb-8 text-p1-text'>
      ...
      <div className='grid grid-cols-2 items-start gap-2'>
        <div className='space-y-2'>
          <Card title='Strikes'>
            <VList>
              {(data.weapons ?? []).filter((w) => isItemWeapon(w.item)).length === 0 ? (
                <VItem>
                  <span className='text-p1-muted'>No equipped weapons.</span>
                </VItem>
              ) : (
                (data.weapons ?? [])
                  .filter((w) => isItemWeapon(w.item))
                  .map((weapon) => {
          ...
          {conditions.length > 0 ? (
            <Card title='Conditions'>
              <VList>
                {conditions.map((c) => (
                  <VItem key={c.name}>{c.value ? `${c.name.toLowerCase()} ${c.value}` : c.name.toLowerCase()}</VItem>
                ))}
              </VList>
            </Card>
          ) : null}
          {rw.immunes.length || rw.resists.length || rw.weaks.length ? (
            <Card title='Defenses'>
              <VList>
                {rw.immunes.length > 0 ? (
                  <VItem>
        ...
        <div className='space-y-2'>
          ...
          <Card title='Perception'>
            <p>
              <span className='font-mono'>{data.proficiencies['PERCEPTION'].total}</span>
              {stringifySenses(data)}
            </p>
          </Card>
          {data.languages.length > 0 ? (
            <Card title='Languages'>
              <div className={`grid gap-x-4 ${data.languages.length > 4 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                {data.languages.map((lang) => (
                  <div key={lang} className='border-b border-p1-border/50 py-0.5'>
                    {toLabel(lang)}
                  </div>
                ))}
              </div>
      ...
      {items.length > 0 || passives.length > 0 ? (
        <div className={`grid items-start gap-2 ${items.length && passives.length ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {items.length > 0 ? (
            <Card title='Items'>
              <div className={`grid gap-x-4 ${items.length > 6 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                {items.map((entry) => (
                  <div
                    key={entry.id}
