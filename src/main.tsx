...
// The default QueryClient uses staleTime: 0, which marks every result stale immediately â€”
...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 1000 * 60, // 1 min â€” long enough to kill remount refetch storms, short enough to stay fresh
...
// then never again â€” after that the precache (revisioned, with cleanupOutdatedCaches)
...
(async () => {
  ...
  try {
    ...
    return; // storage unavailable (private mode) â€” skip rather than wipe every load
  }
  try {
